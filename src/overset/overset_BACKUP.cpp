...
void initialize_receiver(Geometry::StructuredGrid *mygrid, Geometry::StructuredBlock *block) {
  ...
  if (overset_format == "OVERTURE_NOTSCALING") {
    ...
    for (int counter = 0; counter < num_receiverCells[cur_block]; counter++) {
        ...
        receiverCells[counter].igrid = NONE;
        receiverCells[counter].l0 = NONE;
...
//      std::cout << "Rank: " << mpi::irank << ", iblock: " << iblock << ", iblock_donor_4myblock[iblock]: " << iblock_donor_4myblock[iblock] << std::endl;
    ...
    receiverCells = new t_CellReceiver[num_receiverCells_total];
...
void initialize_index4_buf_4donors(void) {
  ...
  int *index4_buf_4receivers;

  for (int irank1 = 0; irank1 < mpi::nprocs; irank1++) { // loop over the entire cores
    if ( mpi::irank == irank1 ) { // this is my turn to send the indices

      for (int irank2 = 0; irank2 < num_receiverRanks_from_mygrid; irank2++) {

        int irank_receiver_thisTurn = irank_receiver_from_mygrid[irank2];
        index4_buf_4receivers = new int[num_receiverCells_from_mygrid[irank2]];
        for (int index = 0; index < num_receiverCells_from_mygrid[irank2]; index++)
          index4_buf_4receivers[index] = NONE;
        ...
        for (int idonor = 0; idonor < num_donorCells_in_this_grid; idonor++) {
          if ( donorCells[idonor].irank_of_receiverCell == irank_receiver_thisTurn ) {
            ...
            index4_buf_4receivers[counter_donorCells] = donorCells[idonor].icell_of_receiverCell;
            counter_donorCells++;

          } // donorCells[idonor].irank_of_receiverCell
        } // idonor
        assert( counter_donorCells == num_receiverCells_from_mygrid[irank2] );

        MPI_Send(index4_buf_4receivers, counter_donorCells, MPI_INT, mpi::irank, irank_receiver_thisTurn, mpi::comm_region);

        DEALLOCATE_1DPTR(index4_buf_4receivers);

      } // irank2

    } // mpi::irank
    else { // all the other ranks receive
      if (num_donorCells_4mygrid > 0) { // if I am receiving anything

        // check if I am receiving at this turn
        int this_rank = NONE;
        for (int irank2 = 0; irank2 < num_donorRanks_4mygrid; irank2++)
          if (irank_donor_4mygrid[irank2] == irank1)
            this_rank = irank2;

        if (this_rank != NONE) {

          index4_buf_4receivers = new int[num_donorCells_4mygrid[this_rank]];

          MPI_Recv(index4_buf_4receivers, num_donorCells_4mygrid[this_rank], MPI_INT, irank1, irank1, mpi::comm_region, mpi::status);

          // fill out index4_buf_4donors
          int counter_base = 0;
          for (int irank2 = 0; irank2 < this_rank; irank2++)
            counter_base += num_donorCells_4mygrid[irank2];
          //
          for (int idonor = 0; idonor < num_donorCells_4mygrid[this_rank]; idonor++)
            index4_buf_4donors[counter_base + idonor] = index4_buf_4receivers[idonor];

          DEALLOCATE_1DPTR(index4_buf_4receivers);

        } // this_rank
      } // num_donorCells_4mygrid
    } // mpi::irank
