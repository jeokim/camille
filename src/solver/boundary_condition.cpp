            ...
            if (myinput->shape_space_inflow == "PLANAR") {
              ...
              if (myinput->model_pde == "LEE_MIXFRAC_CONSTGAMMA") {
                ...
                if (TRUE) { // corresponds to the case as is (without any artificial scaling)
                  double shape_s =  -2.335864 * pow(r_normalized, 6) + 
                                    -4.632679 * pow(r_normalized, 5) + 
                                    22.742211 * pow(r_normalized, 4) + 
                                   -21.767995 * pow(r_normalized, 3) + 
                                     6.550424 * pow(r_normalized, 2) + 
                                    -0.566041 * pow(r_normalized, 1) + 
                                    -0.006550;
                ...
                //  (myboundarydata[IVAR_S])[lb] *= shape_s;
                //} // FALSE
                (myboundarydata[IVAR_UX])[lb] *= 6.762694e-02;
                (myboundarydata[IVAR_UR])[lb] *= 0.0; // just in case
                (myboundarydata[IVAR_UTHETA])[lb] *= 0.0; // just in case
                (myboundarydata[IVAR_P])[lb] *= 6.762694e-02;
                double shape_Z =   32.971644 * pow(r_normalized, 6) + 
                                 -117.421844 * pow(r_normalized, 5) + 
                                  154.872307 * pow(r_normalized, 4) + 
                                  -90.035806 * pow(r_normalized, 3) + 
                                   20.967579 * pow(r_normalized, 2) + 
                                   -1.336852 * pow(r_normalized, 1) + 
                                   -0.025902;
